...
This page is a draft for a future LangChain docs repo integration page. It assumes `langchain-pop` is published as a standalone package with built-in POP object loading, legacy migration support, and LangChain middleware integration.
...
`langchain-pop` accepts legacy POP 0.1 objects directly. The package migrates the legacy object to the canonical POP v1 structure before the middleware uses it.
...
## Relationship to the POP specification

`langchain-pop` is the runtime integration package for the POP object model. It focuses on:

- LangChain middleware
- LangChain agent constructor helper
- built-in POP object loading
- legacy POP 0.1 migration
